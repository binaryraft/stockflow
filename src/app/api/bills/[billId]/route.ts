...
export async function GET(req: NextRequest, { params }: { params: { billId: string } }) {
  const routeName = `[API_BILLS_GET /api/bills/${params.billId}]`;
  ...
  try {
    ...
    if (!billId) {
      console.warn(`${routeName} Bill ID is required.`);
    ...
    if (!bill) {
      console.warn(`${routeName} Bill not found (ID: ${billId}) or does not belong to company (ID: ${companyId}).`);
      return NextResponse.json({ success: false, message: 'Bill not found or does not belong to this company' }, { status: 404 });
    }
    console.log(`${routeName} Bill found and returned successfully.`);
...
export async function PUT(req: NextRequest, { params }: { params: { billId: string } }) {
  const routeName = `[API_BILLS_PUT /api/bills/${params.billId}]`;
  console.log(`${routeName} Received request.`);
  try {
    const { billId } = params;
    ...
    if (paymentStatus === undefined && notes === undefined) {
      console.warn(`${routeName} At least one field (paymentStatus or notes) must be provided for update.`);
    ...
    if (paymentStatus !== undefined && !['paid', 'unpaid'].includes(paymentStatus)) {
      console.warn(`${routeName} Invalid payment status: ${paymentStatus}.`);
    ...
    if (billIndex === -1) {
      console.warn(`${routeName} Bill not found (ID: ${billId}) or not associated with company (ID: ${companyId}).`);
    ...
    if (paymentStatus !== undefined) {
        if ((billToUpdate.type === 'sell' || billToUpdate.type === 'buy') && !billToUpdate.isEstimate) {
            ...
            updated = true;
             ...
             console.warn(`${routeName} Payment status update attempted for bill type '${billToUpdate.type}' (ID: ${billId}) which does not support it or is an estimate.`);
        }
    }
    if (notes !== undefined) {
    ...
    if (updated) {
      ...
      await writeDB(db);
      console.log(`${routeName} Bill details updated successfully.`);
      return NextResponse.json({ success: true, data: billToUpdate });
    } else {
      console.log(`${routeName} No applicable fields to update for bill ID: ${billId}.`);
...
export async function DELETE(req: NextRequest, { params }: { params: { billId: string } }) {
  const routeName = `[API_BILLS_DELETE /api/bills/${params.billId}]`;
  console.log(`${routeName} Received request.`);
  try {
    const { billId } = params;
    const { searchParams } = new URL(req.url);
    ...
    if (!billId) {
      console.warn(`${routeName} Bill ID is required for deletion.`);
    ...
    await writeDB(db);
    console.log(`${routeName} Bill deleted successfully. Note: Stock levels were not automatically readjusted by this operation.`);
