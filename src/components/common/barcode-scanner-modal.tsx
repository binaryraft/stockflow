...
export function BarcodeScannerModal({
  ...
  useEffect(() => {
    ...
    // Define these handlers within useEffect so they capture the correct `isOpen` and `reader`
    const handleMetadataLoaded = () => {
      if (!videoRef.current || !reader || !isOpen) {
        return;
      }
      ...
      currentVideoEl.play().then(() => {
        if (reader && videoRef.current && isOpen) {
    ...
    const startScanner = async () => {
      try {
        ...
        if (!isOpen || !videoRef.current) { // Check again after await
          stream.getTracks().forEach(track => track.stop()); // Release camera if modal closed
          return;
        }
    ...
    return () => {
      stopScanner();
      // Use videoRef.current directly in cleanup to ensure it's the current ref
      if (videoRef.current) {
